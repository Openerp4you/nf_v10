            ...
            if payslip.employee_id.disability not in ('F','P') :
                for line_slip2 in slip_line_pool.search([('slip_id', '=', payslip.id),('code', '=','401')]):
                    for line_pool2 in line_slip2:
                        for tax_ids in pro_obj.search([('state_id','=',payslip.employee_id.location_id.state_id.id)]):
                            for tax in tax_ids:
...
class HrPayslipRun(models.Model):
    ...
    company_id=fields.Many2one('res.company','Zone',default=lambda self: self.env['res.company']._company_default_get('hr.misc.payment'))
