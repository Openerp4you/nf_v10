...
class nf_byod(models.Model):
    ...
    nf_sale_channel = fields.Many2one('employee_id.division_id',string='Division')
    nf_city = fields.Char(string='City')
    ...
    def onchange_employee(self):
        for record in self:
            if record.employee_id:
                self.env.cr.execute("select emp_id,work_email,mobile_phone,department_id,division_id,job_id from hr_employee where id = '"+str(record.employee_id.id)+"'")
                ...
                if tmp: 
                   ...
                   record.nf_sale_channel = tmp[0][4]
